...
IS_NOT_GSI="true"
if [ ${VENDOR_DEVICE} == "lisa" ]; then
	case `uname -r` in
		*android11*)
		echo "IS_NOT_GSI:" ${IS_NOT_GSI}
		IS_NOT_GSI="false"
		;;
	esac
fi
...
for MODULE in ${MODULES}; do
        if [ ${MODULE} == "qca_cld3_wlan" ]; then
			...
			if [ ${VENDOR_DEVICE} == "lisa" ]; then
				if [ ${IS_NOT_GSI} == "false" ]; then
					echo "modprobe foreground:" ${VENDOR_DEVICE}
					${MODPROBE} -a -b -d ${MODULES_PATH} ${MODULE}
				else
					echo "modprobe background:" ${VENDOR_DEVICE}
					${MODPROBE} -a -b -d ${MODULES_PATH} ${MODULE} &
				fi
			else
				${MODPROBE} -a -b -d ${MODULES_PATH} ${MODULE} &
